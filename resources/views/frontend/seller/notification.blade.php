...
@section('content')
    <div id="main" class="site-main">
        <div class="container">
            <div class="row">
                @include('frontend.seller.sidebar-nav',['menu'=>'notification'])
                                ...
                                @if(!empty($notifications[0]))
                                    <table class="shop_table cart wishlist_table">
                                        ...
                                        <tr>
                                            ...
                                            <th class="product-name"><span class="nobr">@lang('messages.seller.description')</span><a href="{{url('/buyer/notification-clear-all')}}" class="btn btn-primary btn-sm pull-right">@lang('messages.seller.clear_all')</a></th>
                                            <th class="" width="70"><span class="nobr">@lang('messages.seller.status')</span></th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        @foreach($notifications as $notification)
                                            <tr>
                                                <td class="product-remove">
                                                    <a href="{{url('/seller/remove-notification/'.$notification->id)}}" class="remove remove_from_wishlist">×</a>
                                                </td>
                                                ...
                                                <td class="status">
                                                    @if($notification->is_viewed == 0) @lang('messages.unread')
                                                    @else @lang('messages.read')
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
        ...
        $(document).ready(function() {
            ...
            $(document.body).on('click','.more',function(e){
                ...
                if(!$(this).hasClass("read")) {
                    $.ajax({
                        type: 'GET',
                        url: '{{url('/seller/notification-read/')}}/'+id,
                        dataType: 'json',
                        context: this,
                        success: function(data){
                            if(data.success == true){
                                $(this).addClass('read');
                                $(this).closest('tr').find('.status').html('@lang('messages.read')');
                            }
                        }
                    }).fail(function(data){
