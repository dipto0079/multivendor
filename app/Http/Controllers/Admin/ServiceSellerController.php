...
{
    public function serviceSellerList(Request $request)
    {
        $search = '';
        ...
        $sellers = $sellers->where('sellers.business_type', ProductTypeEnum::SERVICE)
            ->where('sellers.status', $status)->paginate(env('PAGE_PAGINATE'));

        return view('/admin/seller/service/list')
            ->with('search',$search)
            ->with('status', $status)
            ->with('sellers', $sellers);
    }

    public function serviceSellerSave(Request $request)
    {
        try {
            $seller = new Seller();
            ...
            $service_categories = ProductCategory::where('product_category_level_no', 1)->where('product_category_type_id', 2)->get();
            ...
            if (isset($_GET['edit_id']) || isset($_GET['add_id'])) {
                ...
                if (isset($_GET['add_id'])) {
                    $data_generate .= '<div class="col-sm-6" id=""';
                    $data_generate .= '><div class="form-group"><label class="required">Service Seller Category</label>';
                    $data_generate .= '<select name="category" class="form-control" required';
                    $data_generate .= '>';
                ...
                $commission = Setting::getValueByKey(SettingsEnum::SERVICE_SELLER_COMMISSION);
                ...
                $data_generate .= '<div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <div class="business_email_msg"></div>
                                    </div></div>';
                $data_generate .= '<div class="col-sm-6">
                                <div class="form-group">
                ...
                $data_generate .= ' autocomplete="off"><div class="email_id_msg"></div></div>
                            </div>';
                ...
                $data_generate .= '<div class="row"><div class="col-sm-12">
                                <div class="form-group">
                                    <label class="">About Service Seller</label>
                ...
                if (isset($seller)) {
                    ...
                    if (!isset($encrypted_id)) {
                        ...
                        $seller->business_type = ProductTypeEnum::SERVICE;
                    ...
                    if (!isset($encrypted_id)) {
                        $subscription_fees = Setting::where('key','membership_fee')->first();
                        $form_date = date('Y-m-d');
                        $to_date = date('Y-m-d',strtotime('+1 month'));

                        $subscription = new Subscription();
                        $subscription->from_date = $form_date;
                        $subscription->to_date = $to_date;
                        $subscription->fees = $subscription_fees->value;
    ...
    public function serviceSellerDelete($id)
    {
        try {
            $seller = Seller::where('id', $id)->first();
            $seller_products = $seller->getProducts;
            // if (!empty($seller->getUser->photo) && file_exists('uploads/' . $seller->getUser->photo)) {
            //     unlink('uploads/' . $seller->getUser->photo);
            // }
    ...
    public function serviceSellerStatusChange(Request $request)
    {
        $password = '';
    ...
    public function serviceSellerDetails($seller_id)
    {
      $seller = Seller::find($seller_id);

      return view('/admin/seller/service/details')
          ->with('seller_id', $seller_id)
          ->with('seller',$seller);
    }

    public function serviceSellerServiceList($seller_id)
    {
        ...
        return view('/admin/seller/service/service/list')
            ->with('user_id',$user_id)
            ->with('seller_id', $seller_id)
            ->with('products', $products);
    }

    public function serviceSellerServiceSave(Request $request)
    {
        try {
            ...
            if (isset($_GET['edit_id']) || isset($_GET['add_id'])) {
                ...
                $data_generate .= '<div class="col-sm-12" id="service_type"><div class="form-group">
                                    <label class="">Service Category</label>
                                    <select name="service_type_id" id="service_type_id" class="form-control category_type">';
...
//                                    foreach($product_selected_categories as $product_selected_category){
...
//                                    }
                ...
                $data_generate .= '<div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="ar_description" class="form-control" rows="6">' . $product->ar_description . '</textarea>
                                </div>
                            </div>';
                ...
                if (isset($product)) {
                    ...
                    $product->category_id = $request->service_type_id;
                    $product->payment_required = $request->payment_required;
                    if (!empty($request->seller_id)) $product->seller_id = $seller_id;
...
//            dd($e);
    ...
    public function serviceSellerServiceIsFeatured(Request $request)
    {
        try {
    ...
    public function serviceSellerServiceIsEditorsChoice(Request $request)
    {
        try {
    ...
    public function serviceSellerServiceDelete($id)
    {
        try {
            if (((int)$id) > 0) {
    ...
    public function serviceSellerServiceMediaSave(Request $request)
    {
    ...
    public function serviceSellerServiceMediaDelete()
    {
        try {
    ...
    public function serviceSellerNotificationList($seller_id)
    {
        $seller = Seller::find($seller_id);
        $user_id = $seller->user_id;
        ...
        return view('admin/seller/service/notification/list')
            ->with('user_id',$user_id)
            ->with('seller_id', $seller_id)
    ...
    public function serviceSellerNotificationSave(Request $request)
    {
        try {
            ...
            if (isset($_GET['edit_id']) || isset($_GET['add_id'])) {
                ...
                if (isset($notification)) {
                    ...
                    if (isset($seller_id)) {
                        ...
                        $push_notification_receiver->receiver_id = $seller_id;
                    ...
                    if (isset($seller_id)) $user = User::find($seller_id);
    ...
    public function serviceSellerNotificationDelete($id)
    {
    ...
    // Service Seller Order List
    public function serviceSellerOrderList(Request $request, $seller_id)
    {
...
//        $order_items = OrderItem::wherein('sub_order_id',$order->getSubOrdersArray())->get();
        ...
        $orders = Order::join('sub_orders', 'orders.id', '=', 'sub_orders.order_id')
            ->join('order_items', 'sub_orders.id', '=', 'order_items.sub_order_id')
            ->join('products', 'products.id', '=', 'order_items.product_id')
            ->select(
                'orders.buyer_id',
                'orders.sub_total_price',
                'orders.coupon',
                'orders.discount',
                'orders.processed_by',
                'orders.status',
        ...
        return view('/admin/seller/service/order/list')
            ->with('user_id',$user_id)
            ->with('seller_id', $seller_id)
            ->with('status', $status)
            ->with('orders', $orders);
    ...
    // Service Seller Deal List
    public function serviceSellerDealList(Request $request, $seller_id)
    {
        ...
        return view('/admin/seller/service/deal/list')
            ->with('user_id',$user_id)
            ->with('seller_id', $seller_id)
            ->with('status', $status)
            ->with('deals', $deals);
    ...
    // Service Seller Deal Save
    public function serviceSellerDealSave(Request $request)
    {
        ...
        try {
            ...
            if (isset($_GET['edit_id']) || isset($_GET['add_id'])) {
                ...
                $data_generate .= '<div class="col-sm-12"><div class="form-group">
                                    ...
                                    <input required="" type="text" class="form-control" name="deal_title" value="' . $deal->title . '">';
                $data_generate .= '</div></div>';


                $data_generate .= '<div class="col-sm-6"><div class="form-group">
                                    <label class="required">Discount</label>
                                    <input required="" type="number" class="form-control" name="discount" value="' . $deal->discount . '">';
                $data_generate .= '</div></div>';
    ...
    //Service Seller Deal Change Status
    public function serviceSellerDealChangeStatus(Request $request){
        try{
